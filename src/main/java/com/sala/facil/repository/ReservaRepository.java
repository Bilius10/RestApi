...
public interface ReservaRepository extends JpaRepository<Reserva, Long> {
    ...
    List<Integer> findByusuario_idAndstatus(Long id_usuario, LocalDateTime data);

    @Query("SELECT r.usuario_id FROM Reserva r WHERE r.sala_id = :id AND r.data_pedido = :data AND r.status = 1")
    List<Integer> findByData_pedido(LocalDateTime data, Long id);
    ...
    int updateById_reserva(LocalDateTime data_pedido, LocalDateTime data_reserva, Long id_sala, Long usuario_id, long id_reserva);
}
